...
- **D.3 Assignment and weighting:** Each eligible twin completed every condition. Calibration was fit on the universal clean pool of 1,921 twins. Main and supported moderator means use the same frozen, quota-matched panel of 500 used across all conditions in Tier 1. No post-simulation weights were applied.
...
- **F.2 Aggregation:** Condition means and supported moderator means are arithmetic means of calibrated predictions among the frozen 500-person panel. Twin-2K has no respondents in the benchmark's `gender = Other` level; following organizer guidance, those 221 cells repeat the corresponding condition mean as an explicit no-moderation prediction.

## G · Validation and post-processing

- **G.1 Human validation:** No manual response editing or human outcome ratings.
- **G.2 Parsing and exclusions:** We accepted only sessions whose returned question-ID set exactly equaled the requested set. The production reconstruction matched the Tier 1 export in all 117,000 checked outcome cells. Every submitted cell is unique, nonmissing, and within its declared native scale.
...
- **K.1 Code and materials:** Public code and derived matrices are in this repository. `code/calib/production.py` reconstructs 221 targets, verifies Tier 1 parity, performs calibration, and writes both Tier 2 files. The estimator implementation is vendored under its MIT license in `code/syn-digits/`. The public Zenodo DOI will be added after the repository release.
- **K.2 Raw logs †:** The Silicon simulation request and response archive and SurveyTwin engine snapshot are under restricted access at [10.5281/zenodo.22150315](https://doi.org/10.5281/zenodo.22150315). Public `artifacts/` contains the aligned anchor matrices and the raw and calibrated 1,921 × 221 target matrices needed to reproduce the submitted means.
